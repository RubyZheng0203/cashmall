...
function lastRepayment($binfo){
	...
	$datamoney_x['type'] = 24;
	$datamoney_x['affect_money'] = $affectMoney =  ($binfo['borrow_money']*$_P_fee['money_deposit']/100);
	...
	if($bxid && $x) {
		wqbUnFreezeMoney($binfo['borrow_uid'], "freeze".$binfo['id'] , $affectMoney);
		return true;
	}
	else return false;
}
...
/*function borrowRepayment($borrow_id,$sort_order,$type=1){//type 1:会员自己还,2网站代还
	...
	//企业直投与普通标,判断还款期数不一样
	$voxe = $borrowDetail->field('status,sort_order,sum(capital) as capital, sum(interest) as interest,sum(interest_fee) as interest_fee,deadline,substitute_time')
                        ->where("borrow_id={$borrow_id} and status <> 14")
                        ->group('sort_order')
                        ->select();
	if( $voxe['deadline']>time() && $type==2)  return "此标还没逾期，不用代还";
	$borrowDetail->startTrans();
	$presort =($sort_order>1)? $sort_order-1 : 0;
	foreach($voxe as $ee=>$ss){
		if($ss['sort_order']==$sort_order){ 
			$vo = $ss;
		}
		if($ss['sort_order']==$presort){
			$vopre = $ss;
		}
	}
	if($type==2){ //代还
	    if($presort!=0 && $vopre['status']!=4 && $vopre['status']!=1 && $vopre['status']!=5){
			 return "对不起，此借款第".$presort."期还未代还，请先还第".$presort."期";
		}
	...
	if($is_substitute && $is_expired){//已代还后的会员还款，则只需要对会员的帐户进行操作后然后更新还款时间即可返回
		...
		//逾期了
			$updetail_res = M()->execute( "update `{$pre}investor_detail` set `receive_capital`=`capital` ,`receive_interest`=(`interest`-`interest_fee`),`repayment_time`=".time().",`call_fee`={$call_fee},`expired_money`={$expired_money},`expired_days`={$expired_days},`status`={$idetail_status} WHERE `borrow_id`={$borrow_id} AND `sort_order`={$sort_order} AND status<>14" );
	...
	//企业直投与普通标,判断还款期数不一样
	  $detailList = $borrowDetail->field('invest_id,investor_uid,capital,interest,interest_fee,borrow_id,total')->where("borrow_id={$borrow_id} AND sort_order={$sort_order} AND status<>14")->select();
	//企业直投与普通标,判断还款期数不一样
	...
	//更新还款详情表
	if($type==2){//网站代还
		$updetail_res = M()->execute("update `{$pre}investor_detail` set `receive_capital`=`capital`,`substitute_time`=".time()." ,`receive_interest`=`interest`,`substitute_money`=`substitute_money`+{$total_subs},`status`=4 WHERE `borrow_id`={$borrow_id}  AND status<>14 AND `sort_order`={$sort_order}");
