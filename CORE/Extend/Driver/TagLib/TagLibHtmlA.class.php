...
class TagLibHtmlA extends TagLib{
    ...
    protected $tags   =  array(
        ...
        'file'=>array('attr'=>'id,name,type','close'=>0),
        );
    public function _file($attr)
    {
        $tag        = $this->parseXmlAttr($attr,'file');
        $id         = $tag['id'];
        $name       = $tag['name'];
        $parseStr="";
        $parseStr = '<input type="file" name="'.$id.'" id="'.$id.'" />';
        return $parseStr;
    }
    ...
    public function _editor($attr,$content)
    {
        ...
        switch(strtoupper($type)) {
            case 'ueditor':
                ...
                <textarea name="'.$id.'" id="'.$id.'" type="'.$type.'" style="width:'.$width.';height:'.$height.';'.$style.'">'.$value.'</textarea>
            ...
            case 'KISSY':
			 ...
			 case 'FCKEDITOR':
                ...
                <textarea id="'.$id.'" style="width:'.$width.';height:'.$height.';'.$style.'">'.$value.'</textarea>
                <script type="text/javascript"> 
...
/*****************************新增提现编辑信息扩展功能 添加人：fanyelei 添加时间：2012-12-02 09:10**********************/
	...
	public function _tixianing($attr)
    {
        ...
        $uid      	= $tag['id'];                //文字
        $uname      = $tag['uname'];                //样式名
		$parseStr ="<?php \$id = \$vo['id'];\$_withdraw = M('member_withdraw')->where(array('id'=>\$id))->find(); \$status = \$_withdraw['withdraw_status'];?>";
		$parseStr .= '<a onclick="loadTixianing({$'.$uid.'},\'{$'.$uname.'}\',<?php echo $status;?>)" href="javascript:void(0);">编辑</a>';
        return $parseStr;
    }
