...
class Page {
    ...
    protected $coolPages   ;
    // 分页显示定制
    //protected $config  =	array('header'=>'','prev'=>'上一页','next'=>'下一页','first'=>'第一页','last'=>'最后一页','theme'=>'%header% %upPage% %first%    %linkPage%  %nextPage% %downPage% %end%');
      protected $config  =	array('prev'=>'上一页','next'=>'下一页','first'=>'首页','last'=>'尾页','theme'=>'第%nowPage%页/共%totalPage%页&nbsp;&nbsp; %first% %upPage% %linkPage%  %downPage% %end%');
	//protected $config  =	array('header'=>'条','prev'=>'上一页','next'=>'下一页','first'=>'首页','last'=>'尾页','theme'=>'  %upPage% %linkPage% %downPage%  ');
    // 默认分页变量名
    protected $varPage;
    ...
    public function show() {
		...
		$form = "转到&nbsp;<input type='text' name='pages' id='pages' value='1' class='page_pages' style='width:48px; display: inline-block;color: #949494; margin:0; padding:0; border:#ccc solid 1px; height:21px; line-height:22px;' />&nbsp;页&nbsp;&nbsp;<input type='image' src='__ROOT__/Style/H/images/page_button.gif' class='page_button' name='跳转' onclick='go()' />
...
";
    ...
    public function ajax_show()
    {
        ...
        $pageStr     =     str_replace(
            array('%totalPage%','%upPage%','%downPage%','%first%','%prePage%','%linkPage%'),
            array($this->totalPages,$upPage,$downPage,$theFirst,$prePage,$linkPage),$theme);
        return $pageStr;
    }
